...
public class UtilisateursBean implements Serializable {
  ...
  @Inject private UtilisateurServiceFacade utilisateurServiceFacade;
  ...
  public List<Groupe> completeGroupe(String query) {
    ...
    return groupes.stream()
        .filter(g -> g.getLibelle().toLowerCase().trim().contains(query.toLowerCase().trim()))
  ...
  public void add() {
    ...
    FacesMessage message =
        new FacesMessage(FacesMessage.SEVERITY_INFO, "L'utilisateur a été ajouté", null);
    FacesContext.getCurrentInstance().addMessage(null, message);
    FacesMessage displayPass =
        new FacesMessage(
            FacesMessage.SEVERITY_INFO,
            MessageFormat.format("Le mot de passe généré est : {0}", password),
            null);
    FacesContext.getCurrentInstance().addMessage(null, displayPass);
    FacesMessage messageEmail =
        new FacesMessage(
            FacesMessage.SEVERITY_INFO, "Un e-mail a été envoyé à l'utilisateur", null);
  ...
  public void delete(RetrieveUtilisateursDTO user) {
    ...
    FacesMessage message =
        new FacesMessage(FacesMessage.SEVERITY_INFO, "L'utilisateur a été supprimé", null);
    FacesContext.getCurrentInstance().addMessage(null, message);
    init();
  }
  ...
  public void reinitialiserMotDePasse(RetrieveUtilisateursDTO user) {
    ...
    FacesMessage displayPass =
        new FacesMessage(
            FacesMessage.SEVERITY_INFO,
            MessageFormat.format("Le mot de passe a été réinitialisé : {0}", password),
            null);
    FacesContext.getCurrentInstance().addMessage(null, displayPass);
    FacesMessage messageEmail =
        new FacesMessage(
  ...
  public void lock(RetrieveUtilisateursDTO user) {
    ...
    FacesMessage lockMessage =
        new FacesMessage(
            FacesMessage.SEVERITY_INFO,
            MessageFormat.format(
                "L''utilisateur {0} {1} a été verrouillé et ne pourra plus se connecter",
  ...
  public void onCellEdit(CellEditEvent event) {
    ...
    if (newValue != null && !newValue.equals(oldValue)) {
      ...
      FacesMessage msg =
          new FacesMessage(FacesMessage.SEVERITY_INFO, "L'utilisateur a été modifié", null);
