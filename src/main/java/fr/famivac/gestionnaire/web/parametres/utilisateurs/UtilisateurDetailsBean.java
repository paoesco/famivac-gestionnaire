...
public class UtilisateurDetailsBean implements Serializable {
  ...
  public void init() {
    ...
    //        groupes = groupesDTO.stream().map(groupe ->
    // groupe.getNom()).collect(Collectors.toList());
  }
  ...
  public void sauver() {
      ...
      if (!form.getPassword().equals(form.getConfirmPassword())) {
        FacesContext.getCurrentInstance()
            .addMessage(
                "form:form_password",
                new FacesMessage(
                    FacesMessage.SEVERITY_ERROR, "Les mots de passe ne correspondent pas", ""));
        ((UIInput)
                FacesContext.getCurrentInstance().getViewRoot().findComponent("form:form_password"))
            .setValid(false);
        ((UIInput)
                FacesContext.getCurrentInstance()
                    .getViewRoot()
                    .findComponent("form:form_confirm_password"))
            .setValid(false);
        return;
      }
    }
    ...
    // FacesMessage(FacesMessage.SEVERITY_INFO, "Les informations ont été enregistrées", ""));
  }
