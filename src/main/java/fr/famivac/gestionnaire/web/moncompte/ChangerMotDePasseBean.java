  ...
  public void changer() {
    if (!newPassword.equals(confirmNewPassword)) {
      FacesMessage message =
          new FacesMessage(FacesMessage.SEVERITY_WARN, "Les mots de passe ne correspond pas", null);
      FacesContext.getCurrentInstance().addMessage(null, message);
      return;
    }
    try {
      utilisateurService.changePassword(
          sessionBean.getConnectedUser().getLogin(), actualPassword, newPassword);
      actualPassword = "";
      newPassword = "";
      confirmNewPassword = "";
      FacesMessage message =
          new FacesMessage(FacesMessage.SEVERITY_INFO, "Votre mot de passe a été changé", null);
      FacesContext.getCurrentInstance().addMessage(null, message);
    } catch (WrongCredentialsException ex) {
      Logger.getLogger(ChangerMotDePasseBean.class.getName()).log(Level.FINE, null, ex);
      FacesMessage message =
          new FacesMessage(
              FacesMessage.SEVERITY_WARN, "Le mot de passe actuel ne correspond pas", null);
      FacesContext.getCurrentInstance().addMessage(null, message);
    }
  }
