...
@NamedQueries({
  @NamedQuery(
      name = Sejour.QUERY_SEJOURS_DE_LA_FAMILLE,
      query = "select s from Sejour s where s.familleId = :familleId order by s.dateDebut"),
  @NamedQuery(
      name = Sejour.QUERY_SEJOURS_DE_L_ENFANT,
      query = "select s from Sejour s where s.enfantId = :enfantId order by s.dateDebut"),
  @NamedQuery(
      name = Sejour.QUERY_SEJOURS_RETRIEVE,
      query = "select s from Sejour s order by s.dateDebut, s.dateFin"),
  @NamedQuery(
      name = Sejour.QUERY_SEJOURS_TERMINES_DANS_LA_PERIODE,
      query = "select s from Sejour s where :dateDebut <= s.dateFin and s.dateFin <= :dateFin")
})
public class Sejour implements Serializable {
  ...
  public static final String QUERY_SEJOURS_TERMINES_DANS_LA_PERIODE =
      "querySejoursTerminesDansLaPeriode";
  ...
  protected Sejour() {
    forfaitJournalier = BigDecimal.ZERO;
    fraisDossier = BigDecimal.ZERO;
    fraisVoyage = BigDecimal.ZERO;
    fraisPensionFamilleJournalier = BigDecimal.ZERO;
    payeurs = new HashSet<>();
  }
  ...
  public Sejour(
    ...
    periodeJourneeDateDebut = periodeJourneeDebut;
    this.dateFin = (Date) dateFin.clone();
    periodeJourneeDateFin = periodeJourneeFin;
  }
  ...
  public Sejour withFraisSejourJournalier(BigDecimal fraisSejourJournalier) {
    forfaitJournalier = fraisSejourJournalier;
    return this;
  }
  ...
  public BigDecimal getFraisPensionFamilleJournalier() {
    return Objects.isNull(fraisPensionFamilleJournalier)
        ? BigDecimal.ZERO
