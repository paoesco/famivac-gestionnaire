...
public class SejourService {
  ...
  public List<SejourDTO> get() {
    List<Sejour> entities =
        entityManager.createNamedQuery(Sejour.QUERY_SEJOURS_RETRIEVE, Sejour.class).getResultList();
  ...
  public List<SejourDTO> rechercher(
      String nomReferent,
  ...
  public void updateEnfant(@Observes UpdateEnfantEvent pEvent) {
    ...
    List<Sejour> sejours =
        entityManager
            .createNamedQuery(Sejour.QUERY_SEJOURS_DE_L_ENFANT, Sejour.class)
  ...
  public List<VoyageDTO> prochainsVoyages(Date date) {
    return voyageRepository.prochainsVoyages(date).stream()
        .map(
            voyage -> {
              VoyageDTO dto = new VoyageDTO();
              String accompagnateurs =
                  voyage.getAccompagnateurs().stream()
                      .map(
                          acc -> {
              ...
              // Heure rdv est automatiquement calculée 30 minutes avant l'heure officielle de
              // départ du transport
              ...
              String accompagnateursTelephones =
                  voyage.getAccompagnateurs().stream()
                      .map(
                          acc -> {
  ...
  public BilanDTO getBilanSurLaPeriode(Date dateDebut, Date dateFin) {
    ...
    for (Sejour sejour : sejours) {
      ...
      totalFraisPensionFamille =
          totalFraisPensionFamille.add(
              new BigDecimal(nombreJourneesVacancesSejour)
                  .multiply(sejour.getFraisPensionFamilleJournalier()));
    }
